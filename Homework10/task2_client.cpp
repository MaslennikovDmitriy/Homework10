...
int main()
{
	...
	try
	{
		...
		boost::asio::ip::tcp::socket name_socket(io_service, endpoint.protocol());
		boost::asio::ip::tcp::socket message_socket(io_service, endpoint.protocol());

		name_socket.connect(endpoint);
		message_socket.connect(endpoint);
